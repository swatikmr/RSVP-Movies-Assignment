...
SELECT median_rating, COUNT(r.movie_id) AS movie_count
FROM ratings r
INNER JOIN movie m
ON m.id = r.movie_id
...
INNER JOIN ratings r 
ON m.id = r.movie_id
WHERE languages LIKE '%German%') AS German_votes,
(SELECT SUM(total_votes)
FROM movie m
INNER JOIN ratings r 
ON m.id = r.movie_id
WHERE languages LIKE '%Italian%') AS Italian_votes;
...
SELECT n.name AS actress_name, SUM(r.total_votes) AS total_votes, COUNT(m.id) AS movie_count,
				...
				RANK() OVER(ORDER BY ROUND(SUM(r.avg_rating*r.total_votes)/SUM(total_votes),2) DESC) AS actress_rank
FROM movie m
INNER JOIN ratings r
ON m.id = r.movie_id
INNER JOIN role_mapping rm 
